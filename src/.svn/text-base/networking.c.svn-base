...
static void acceptCommonHandler(int fd) {
    ...
    // 它分配并初始化了一个客户端对象
    // 选择数据库 0 （默认的）
    // 通过accept（2）赋予客户端一个文件描述符
    // 最终这个客户端被加入全局客户端列表中
    // 在 createClient 里server注册了一个hander事件在even loop中
    // 当有数据从这个客户端出现的时候
    ...
    // 如果定义了 maxclients 的限制，那么超过这个限制的客户端会被写入一个错误
    // 信息，然后释放，并从客户端队列中移除
    ...
    while(c->multibulklen) {
        ...
        if (c->bulklen == -1) {
            ...
            if (newline != NULL) {
            	...
            	// 这里应该是等待更多的数据，redis其实用/r/n作为标识符
                break;
            }
        } // if (newline != NULL) {
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        // 如果已经 reply 了， 那也什么都不做
        if (c->flags & REDIS_CLOSE_AFTER_REPLY) return;

        /* Determine request type when unknown. */
        // 整个query完成了，确定 request 类型
        if (!c->reqtype) {
            if (c->querybuf[0] == '*') {
...
void readQueryFromClient(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    // 读取成功，nread 表述读取的字节数
    if (nread) {
    	// 将 buf copy 到该客户端指定的 c->querybuf 中
    	// 这个是一个 cat 操作，而不是纯 copy 操作
