...
void hashTypeTryConversion(robj *subject, robj **argv, int start, int end) {
    ...
    // 如果没有配置 默认 hash_max_zipmap_value ： REDIS_HASH_MAX_ZIPMAP_VALUE 即 64
    for (i = start; i <= end; i++) {
...
int hashTypeSet(robj *o, robj *key, robj *value) {
    ...
    if (o->encoding == REDIS_ENCODING_ZIPMAP) {
        ...
        // 删除这个 key 和 value 的副本
        decrRefCount(key);
        decrRefCount(value);

        /* Check if the zipmap needs to be upgraded to a real hash table */
        // 除了key 和value 大于某个定值外
        // key-value 对的数目大于某一个值也可以生成一个真正的hashtable
...
void hsetCommand(redisClient *c) {
    ...
    // argv[1] 表示的是 key，可以简单看作 hashtable 的表名
    if ((o = hashTypeLookupWriteOrCreate(c,c->argv[1])) == NULL) return;
    // 是否需要转换成真正的hash table
    hashTypeTryConversion(o,c->argv,2,3);

    // 看看能否转变编码，一个string可以被编码为int，这样节省空间
